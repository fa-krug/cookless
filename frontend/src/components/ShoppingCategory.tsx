...
export default function ShoppingCategory({ category, items, onToggleItem }: ShoppingCategoryProps) {
  ...
  return (
    <Collapsible defaultOpen className="group rounded-lg border border-border bg-card shadow-sm">
      ...
      <CollapsibleContent>
        <div className="divide-y divide-border border-t border-border">
          {sortedItems.map((item) => (
            <label
              key={item.id}
              className="flex cursor-pointer items-center gap-3 px-4 py-2.5 hover:bg-muted"
            >
              <Checkbox
